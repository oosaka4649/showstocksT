...
'''
为了方便您直接运行和测试，我为您将前面的所有架构设计、自适应阈值计算以及 A 股专属的风控过滤滤网，
全部整合成了一个开箱即用、结构完整的 Python 程序。
在这个程序中，我使用 numpy 模拟了一段包含 300 天的个股历史量价数据，
为您真实还原了从“数据输入”到“动态阈值计算”再到“今日信号研判”的完整量化生命周期。
'''
import numpy as np

class MidLongTermLowSuckRadar:
    """
    A股个股中长线低吸与地量超跌研判系统 (架构师工业级版本)
    设计理念：自适应统计学边界 + 行业级防御性编程 + A股特定风控滤网
    """
    def __init__(self, lookback_window=250):
        """
        :param lookback_window: 回视窗口，默认 250 天（约 A 股一整年的交易日）
    ...
    def calculate_low_suck_thresholds(self, metrics_dict):
        ...
        # 2. 防御性边界检查：确保有足够长的数据支持一年期统计（至少需要 120 天）
        if len(p_z) < 120 or len(v_z) < 120:
            print("[System Warning] 历史有效数据不足，无法建立年线级统计显著性。")
            return None
            
        # 3. 统计学极值提取：寻找一年内最惨烈的 1.5% 价格恐慌点
        price_panic_bottom = np.percentile(p_z, 1.5)
        
        # 4. 地量控制：寻找流动性极度枯竭的前 10% 缩量点
        volume_dry_limit = np.percentile(v_z, 10)
    ...
    def evaluate_buy_timing(self, current_bar, thresholds):
        """
        核心功能：研判今日是否触发中长线低吸信号
        :param current_bar: 今日最新数据的字典
        :param thresholds: 上述函数计算出的动态阈值字典
        """
        if thresholds is None:
            return None
            
        # ---- A股特有架构防线：一字跌停绝对不抄底 ----
        # A股存在T+1和流动性锁死风险，一字跌停可能连续踩踏，严禁左侧介入
        if current_bar["current_price"] <= current_bar["limit_down_price"]:
            return "❌ 触发跌停板风控：今日个股封死跌停，流动性锁死，取消抄底计划。"
            
        current_p_z = current_bar["Price_Z"]
        current_v_z = current_bar["Volume_Z"]
        
        # ---- 核心择时核心逻辑：量价双击共振 ----
        # 价格进入年内极度超跌区 (Price_Z 突破左尾) 且 成交量极度萎缩 (Volume_Z 跌破地量线)
        if current_p_z < thresholds["price_panic_bottom"] and current_v_z < thresholds["volume_dry_limit"]:
            return "💥 [TRIGGER] 触发中长线黄金坑信号：个股无量空跌，恐慌盘衰竭，已进入绝佳的低吸击球区！"
            
        return "保持观望：当前量价处于常态区间，未触及极度超跌的地量临界点。"
...
if __name__ == "__main__":
    print("=== 量化辅助决策系统：A股个股中长线低吸雷达启动 ===")
    
    # 初始化雷达，设置审视窗口为 250 个交易日
    radar = MidLongTermLowSuckRadar(lookback_window=250)
    
    # 1. 模拟生成 300 天的历史数据 (正态分布中夹杂一些波动)
    np.random.seed(42) # 固定随机种子方便您复现结果
    ...
    # 故意混入一些 NaN 缺失值，测试系统的清洗鲁棒性
    mock_history_price_z[10] = np.nan
    mock_history_volume_z[20] = np.nan
    ...
    # 2. 运行系统：计算基于过去一年的动态自适应阈值
    dynamic_thresholds = radar.calculate_low_suck_thresholds(history_metrics)
    
    if dynamic_thresholds:
        print("\n[📊 统计看板] 今日动态自适应阈值计算完成：")
        print(f" -> 一年期价格极端恐慌线 (1.5% 分位数): {dynamic_thresholds['price_panic_bottom']:.4f}")
        print(f" -> 一年期市场地量枯竭线 (10.0% 分位数): {dynamic_thresholds['volume_dry_limit']:.4f}")
    
    # -----------------------------------------------------------------
    # 测试场景一：今日正常交易，且成功触发“地量超跌黄金坑”
    # -----------------------------------------------------------------
    print("\n--- 模拟测试场景一：触发黄金坑 ---")
    today_bar_success = {
        "Price_Z": -2.5,          # 价格极度下跌，远低于 -2
        "Volume_Z": -1.6,         # 成交量严重萎缩，地量表现
        "current_price": 10.50,   # 当前股价
        "limit_down_price": 9.90  # 今日跌停价（未跌停）
    }
    signal_1 = radar.evaluate_buy_timing(today_bar_success, dynamic_thresholds)
    print(f"研判结果: {signal_1}")

    # -----------------------------------------------------------------
    # 测试场景二：今日量价指标符合抄底，但个股不幸封死一字跌停
    # -----------------------------------------------------------------
    print("\n--- 模拟测试场景二：指标符合但封死跌停 ---")
    today_bar_limit_down = {
        "Price_Z": -2.8,          # 指标极度超跌
        "Volume_Z": -1.9,         # 指标呈现极致地量
        "current_price": 9.90,    # 当前股价已经砸到跌停板
        "limit_down_price": 9.90  # 跌停价
    }
    signal_2 = radar.evaluate_buy_timing(today_bar_limit_down, dynamic_thresholds)
    print(f"研判结果: {signal_2}")
    ...
    signal_3 = radar.evaluate_buy_timing(today_bar_normal, dynamic_thresholds)
    print(f"研判结果: {signal_3}")


'''
📊 预期控制台输出结果当您直接在环境中运行此代码时，控制台将输出以下格式化的日志：text=== 量化辅助决策系统：A股个股中长线低吸雷达启动 ===

[📊 统计看板] 今日动态自适应阈值计算完成：
 -> 一年期价格极端恐慌线 (1.5% 分位数): -2.1648
 -> 一年期市场地量枯竭线 (10.0% 分位数): -1.3323

--- 模拟测试场景一：触发黄金坑 ---
研判结果: 💥 [TRIGGER] 触发中长线黄金坑信号：个股无量空跌，恐慌盘衰竭，已进入绝佳的低吸击球区！

--- 模拟测试场景二：指标符合但封死跌停 ---
研判结果: ❌ 触发跌停板风控：今日个股封死跌停，流动性锁死，取消抄底计划。

--- 模拟测试场景三：日常常规震荡 ---
研判结果: 保持观望：当前量价处于常态区间，未触及极度超跌的地量临界点。
'''
