...
class VP_SignalGenerator:
    ...
    def generate_signals(self, metrics_dict):
       ...
       # 市场象限分类： quadrant
        ...
        buy_left_mask_all = (quadrant == 3) & ((p_z < self.v_bottom_threshold) | (v_z < self.v_volume_threshold)) & below_long_ma
        combined_signals[buy_left_mask_all] = 1
        signal_labels[buy_left_mask_all] = "BUY_左侧爆量抄底 new 1"    

        buy_left_mask_all1 = (quadrant == 4) &  (v_z < self.v_volume_threshold) & below_long_ma
        combined_signals[buy_left_mask_all1] = 1
        signal_labels[buy_left_mask_all1] = "BUY_左侧爆量抄底 成功率低，放地量时成功率高"                
...
class VP_QuantRunner(VP_QuantRunner_BaseModel):
    def __init__(self, p_window=15, v_window=15, v_bottom=-2.0, v_volume=2.5, momentum_cos=0.8, divergence_vol=-1.0, ma_short=20, ma_long=60, lookback_window=150):
    ...
    def run_pipeline(self, stock_data):
        ...
        report['quant_info'] = f"backtest test 2， 左侧交易为主，成绩最好，当和其他策略同时出现时"
    ...
    def _print_markdown_report(self, report):
        print("\n" + "="*80)
        print(f"ai quant backtest test 2   左侧抄底 + 趋势生命线防线 终极绩效看板")
        print("="*80)
        markdown_output = f"""
...
例如，当价格跌破 v_bottom（极度超跌），但成交量却萎缩到了 divergence_vol（无量空跌），这在技术分析中是极强的地量见地价、企稳反弹的信号。
 ...
 '''
    ...
    def customize_thresholds(self, metrics_dict):
        ...
        #在成交量（Volume）的统计序列中，数值越小代表成交量越干枯（地量）
        #从纯粹的非参数统计学（Non-parametric Statistics）定义来看，8% 的分位数绝对比 10% 更苛刻。
        #10% 阈值：意味着过去 100 天中，有 10 天可以满足条件。
        # 8% 阈值：意味着过去 100 天中，只有 8 天最干枯的日子能满足条件。它在过滤信号时收紧了 20% 的准入门槛。
        volume_dry_limit = np.percentile(v_z, 6.0),  # 地量卡得更死（前7%）
