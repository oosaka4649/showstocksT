...
def main():
    ...
    def create_sample_data():
        ...
        for date in dates:
            ...
            if date.weekday() < 5:  # 周一到周五
                ...
                price = max(price + change, 1)  # 确保价格为正
                data.append({
        ...
        print("示例数据已保存为 'sample_stock_data.csv'")
        return 'sample_stock_data.csv'
    
    # 使用示例
    csv_file = 'D:\python\showstocksT\cvs_search_app\stockscsv\sz300403.csv'  # 替换为您的CSV文件路径
    
    # 指定起始日期（例如："03-01"表示每年从3月1日开始）
    start_date = "10-01"  # 可以修改为任何想要的起始日期，如"01-01", "07-01"等
    ...
    try:
        ...
        print(f"文件 {csv_file} 未找到，正在生成示例数据...")
