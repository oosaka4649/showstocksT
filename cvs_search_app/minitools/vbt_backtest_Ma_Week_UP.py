...
这次，对 VectorbtBacktest_DayWeek 进行修改，就是用短期上穿长期，看看效果怎么样
    对出场规则进行了修改，就是当日股价下穿5日均线
    添加一个方法，将股票的全部数据可以截取从指定日期以后的
...
class VectorbtBacktest_DayWeek_UP(StockMA_Strategy):
    def __init__(self, stock_code: str, startDay=None):
        super().__init__(stock_code, startDay)
    ...
    def simple_backtest(self):
        ...
        if float_close_list is not None:
            ...
            try:
                ...
                #entries = slow_ma_w.close_crossed_above(fast_ma.ma) # 5日上穿周 买
                entries = close_indicator_obj.close_crossed_above(slow_ma_w.ma) # 5日上穿周 买
                exits = close_indicator_obj.close_crossed_below(fast_ma.ma) #股价下穿5日均线卖
...
if __name__ == "__main__":
    ...
    strategy_instance = VectorbtBacktest_DayWeek_UP(stock_code)
