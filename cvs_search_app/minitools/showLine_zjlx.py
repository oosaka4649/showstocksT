...
def running_sum(nums):
    return_sum = []
    return_sum.append(nums[0])
    for i in range(1, len(nums)):
        return_sum.append(return_sum[i-1] + nums[i])
    return return_sum
...
def line_rzrq_sh_sz_value(zjlx_data) -> Line:
    c = (Line()
        .add_xaxis(xaxis_data=zjlx_date)
        .add_yaxis(
            series_name="主力净流入",
            y_axis=running_sum(zjlx_data['main']),
            is_smooth=True,
        ...
        .add_yaxis(
            series_name="大单净流入",
            y_axis=running_sum(zjlx_data['large']),
            is_smooth=True,
        ...
        .add_yaxis(
            series_name="超大单净流入",
            y_axis=running_sum(zjlx_data['super_large']),
            is_smooth=True,
            is_connect_nones=True,
            linestyle_opts=opts.LineStyleOpts(width=3, opacity=0.5),
        ...
        .add_yaxis(
            series_name="中单净流入",
            y_axis=running_sum(zjlx_data['medium']),
            is_smooth=True,
            is_connect_nones=True,
            linestyle_opts=opts.LineStyleOpts(width=3, opacity=0.5),
            itemstyle_opts=opts.ItemStyleOpts(color="#22D44E"),  # 添加这一行定义颜色
        ...
        .add_yaxis(
            series_name="小单净流入",
            y_axis=running_sum(zjlx_data['small']),
            is_smooth=True,
            is_connect_nones=True,
            linestyle_opts=opts.LineStyleOpts(width=3, opacity=0.5),
