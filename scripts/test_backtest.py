...
# 文件路径
file_path = 'E:\\python\\cvs_search_app\\stockscsv\\sz300189.csv'
...
if close is not None:
    ...
    try:
        ...
        entries = fast_m_a.ma_crossed_above(slow_ma)
        exits = slow_m_a.ma_crossed_below(slow_ma)
        pf = vbt.Portfolio.from_signals(close, entries, exits, init_cash=100000)
        '''
        print(pf.total_return())  # 输出总收益率
        pf.total_profit()
        pf.plot().show()    
        print(fig.total_return())  # 输出总收益率
        fig.total_profit()        
        '''
        print('1---')
        ...
        pf.stats().to_csv('E:\\python\\cvs_search_app\\stockscsv\\backtest_results.csv')
        # Accessing trade details
        trades = pf.trades.records_readable
        print("\nTrade Details:")
        trades.to_csv('E:\\python\\cvs_search_app\\stockscsv\\trade_details.csv')
        pf.plot().show()
    except Exception as e:
        print(f"加载数据失败: {e}")
else:
